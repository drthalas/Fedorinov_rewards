...
class Ale263ReferenceRankInsigniaTests(unittest.TestCase):
    ...
    def test_scoped_templates_scripts_and_styles_expose_required_ui(self) -> None:
        ...
        self.assertIn("window.FedorinovPhotoPlusFlow", preview_script)
        self.assertIn("plusFlow.shouldOpenFilePicker(trigger)", preview_script)
        self.assertIn("plusFlow.markClipboardSuccess(trigger)", preview_script)
        self.assertNotIn("clipboardAttempted", preview_script)
