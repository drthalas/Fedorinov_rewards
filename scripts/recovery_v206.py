...
def select_installation(candidates: list[InstallationCandidate], ui: RecoveryUI) -> InstallationCandidate:
    ...
    if len(candidates) == 1:
        ...
        if selected is None:
            raise RecoveryCancelled("Выбранная папка не является поддерживаемой установкой v2.0.5. Изменения не выполнялись.")
...
def load_recovery_package(service_dir: Path) -> RecoveryPackage:
    ...
    if package_path.parent != service_dir.resolve(strict=False) or not package_path.is_file():
        raise RecoveryError("Основной пакет v2.0.6 не найден в recovery-архиве.")
    actual_size = package_path.stat().st_size
    actual_sha = sha256_file(package_path)
    if actual_size != expected_size or actual_sha != expected_sha:
        raise RecoveryError("Основной пакет v2.0.6 повреждён: размер или SHA256 не совпадает.")
...
def validate_and_extract_package(package: RecoveryPackage, destination: Path) -> Path:
    ...
    try:
        with ZipFile(package.zip_path) as archive:
            ...
            if archive.testzip():
                raise RecoveryError("Основной пакет v2.0.6 повреждён.")
        ...
        raise RecoveryError("Основной пакет v2.0.6 не является корректным ZIP.") from exc
    ...
    *,
    ...
    try:
        ...
        if app_name != PRODUCT_NAME or installed_version != package.version:
            raise RecoveryError("Установленная программная часть не подтверждает версию v2.0.6.")
        ...
        if rollback_errors:
            raise RecoveryError(
                f"Восстановление v2.0.6 не удалось ({exc}); rollback требует помощи: " + " | ".join(rollback_errors)
            ) from exc
        raise RecoveryError(
            f"Восстановление v2.0.6 не удалось. Предыдущая v{candidate.version} восстановлена и запущена; данные сохранены."
        ) from exc
...
def run_recovery(service_dir: Path, ui: RecoveryUI | None = None) -> dict[str, object]:
    ...
    interface.show("Восстановление завершено. Версия 2.0.6 запущена, база и фотографии сохранены.")
    webbrowser.open(f"http://{candidate.host}:{candidate.port}")
    return result


def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Safe one-time Fedorinov Rewards v2.0.6 recovery.")
