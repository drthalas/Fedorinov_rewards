...
with patch.object(updater, "check_for_updates", return_value=manifest("2.0.1", candidate_zip)):
...
if not result.get("ok"):
    raise AssertionError(f"v2.0.1 legacy apply failed: {result}")
...
if "APP_VERSION = \"2.0.1\"" not in (legacy_root / "backend" / "app" / "version.py").read_text(encoding="utf-8"):
    raise AssertionError("installed version source is not 2.0.1")
...
manifest = json.dumps({'version': '2.0.1', 'download_url': 'https://example.test/v2.0.1.zip', 'sha256': 'a'*64}).encode()
...
if restart_result != {"version": "2.0.1", "update_available": False}:
...
if [item["destination"] for item in downloads] != [
    ...
    "FedorinovRewards_WebPreview_v2.0.1.zip",
]:
