...
def plan_builder(_settings, current):
    return UpdatePlan(current, "2.0.6", True, "https://example.test/v2.0.6.zip", candidate_sha, ["v2.0.6"])
...
identity = fetch_runtime_identity("127.0.0.1", port, 0.5)
if not identity or identity.get("version") != "2.0.6":
    raise AssertionError(f"public v2.0.5 updater did not start v2.0.6: {identity}")
...
if not repeat_evidence.get("reused") or repeat_evidence.get("pid") != identity.get("pid"):
    raise AssertionError(f"repeat launch did not reuse v2.0.6: {repeat_evidence}")
healthy = [item for item in supervisor.inspect_all() if item.confirmed and item.healthy]
if len(healthy) != 1 or healthy[0].record.pid != identity["pid"]:
...
spec = importlib.util.spec_from_file_location("exact_recovery", service_dir / "recovery_v206.py")
...
identity = module._fetch_identity(candidate.host, candidate.port, 0.5)
if not identity or identity.get("version") != "2.0.6":
    raise AssertionError(f"recovery did not start v2.0.6: {identity}")
...
def _safe_extract_recovery(zip_path: Path, destination: Path) -> Path:
    with ZipFile(zip_path) as archive:
        ...
        required = {"service/recovery_v206.py", "service/manifest.json"}
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Verify exact public v2.0.5 updater and v2.0.6 recovery contracts.")
