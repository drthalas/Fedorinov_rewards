...
Это portable preview web-версии Fedorinov Rewards для первичной проверки на Windows.
...
По умолчанию preview запускается безопасно:
...
В этом режиме можно смотреть данные, фотографии, справочники и карточки, но кнопки изменения не должны использоваться для записи.

## Проверка кнопок изменения

Включать write mode можно только для временного теста на копии данных и только после backup.

1. Сделайте backup папки Rewards.
2. Убедитесь, что backup можно открыть и восстановить.
3. В `.env` временно укажите:
...
4. Запустите `start_windows.bat`.
5. После проверки верните:
...
- Не включать `WRITE_MODE=true` на единственной рабочей базе без backup.
