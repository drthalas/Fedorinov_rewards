...
Updater/product/physical проверки exact package выполняются в release-candidate
gate до публикации. После PASS и фиксации SHA/version/artifact SHA256 publication
не повторяет этот updater cycle. Обновление `Public Current` — отдельная
вспомогательная операция, а не основание снова выполнять acceptance.
