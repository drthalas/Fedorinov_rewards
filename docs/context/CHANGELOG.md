...
- Исправлен запуск recovery-архива на стандартной русской Windows: короткий BAT больше не содержит сложной логики и русских строк, которые мог неверно разобрать `cmd.exe`.
- Основное восстановление выполняет app-owned helper с понятными русскими сообщениями.
- Перед изменением по-прежнему требуется явный выбор установки и создаётся проверенный backup.
- Обновляется только выбранная установка; DB, фотографии и документы сохраняются.
- При ошибке программная часть откатывается, а пользовательские данные остаются неизменными.
- Исправленный recovery публикуется отдельным архивом `v2.0.7`; опубликованный архив `v2.0.6` не перезаписывается.

Проверки перед релизом:

- Exact public `v2.0.6` parser failure и corrective bootstrap проверены в native Windows `cmd.exe`.
- Пути с пробелами и кириллицей, Desktop/Downloads и Windows code pages: PASS.
- Выбор одной/нескольких установок, folder picker, cancel, invalid target, backup, success и forced rollback: PASS.
- Exact public `v2.0.5` recovery/update contract, full suite, package safety и TEMP packaged Browser smoke: PASS.
