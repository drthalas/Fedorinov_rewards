...
- Добавлен read-only анализ состава и объёма managed media без изменения исходных данных.
- Оптимизация создаёт отдельную рабочую копию и преобразует только безопасные JPEG-кандидаты по принятой политике качества без изменения разрешения.
- Индекс повторных запусков обрабатывает только новые, изменённые или отсутствующие файлы и поддерживает безопасное продолжение после прерывания.
- В разделе «Обслуживание данных» доступны проверка, оптимизация, прогресс, health status и итог «Было / Стало / Сэкономлено».
- Активация optimized copy выполняется только после SQLite, FK, reference, path и media decode health checks.

Проверки release candidate:

- Полный integrated flow выполнен на Windows VM Sergey-full: анализ, optimized copy, конвертация, health, активация, product smoke и incremental runs.
- Source dataset остался неизменным; фактическая экономия составила 26,359 GB (44,71%).
- Production tag, GitHub Release, `latest.json` и Telegram заблокированы до отдельного Owner-разрешения.
...
- Восстановлена быстрая фильтрация кавалеров на больших базах с канонической сортировкой и сохранением фильтра при групповой навигации.
- Create-flow кавалера поддерживает фотографии и последовательное добавление нескольких наград до финального сохранения без orphan-данных.
- Состояние вкладки «Поиск» сохраняется только в текущем сеансе; «Сводная таблица» использует постраничную выдачу без изменения результатов.
- В списках наград используются изображения из справочника; отсутствующие изображения не создают broken-image.
- Clipboard-first добавление фотографий, каскадный выбор награды и автоматическая справочная ссылка работают в общем integrated flow.
- Сохранены плавные переходы без белых кадров.

Проверки release candidate:

- Accepted product HEAD интегрирован в `main` после exact ancestry/parity verification.
- Exact package проходит static/safety/parity checks и штатное UI-обновление из public `v2.0.12` на Windows VM.
- Physical Windows не используется на release-candidate stage; ручной Owner gate выполняется только после технического VM PASS.
- Production tag, GitHub Release, `latest.json` и Telegram заблокированы до отдельного Owner-разрешения.
...
- Наименования наград отсортированы по алфавиту и фильтруются по началу введённого текста.
- Поле звания или специальности поддерживает такой же поиск по первым буквам без нерелевантных вариантов.
- Производные справочные сведения награды подставляются из выбранного наименования и доступны только для чтения.
- Поля конкретного экземпляра награды, фотографии и документы сохраняют прежний режим редактирования.

Проверки release candidate:

- Accepted product HEAD интегрируется в `main` только после exact ancestry/parity verification.
- Повторный полный Windows VM Tier 4 gate не выполняется по разовому Owner-разрешению ALE-374.
- Exact merged candidate проходит штатное UI-обновление из public `v2.0.11` и post-update проверку на physical Windows.
- Production publication, `latest.json` и Telegram заблокированы до отдельной команды Owner.
...
- В список кавалеров добавлена цифровая навигация `1–9` слева от алфавитной полосы с общей групповой загрузкой и сохранением поиска и выбора.
- В отдельной карточке кавалера продублированы действия открытия каталога, архивирования и формирования буклета.
- Восстановлено надёжное открытие каталога в видимой Windows-сессии с конечным success/error состоянием.
- Главный экран сохранён эквивалентным предыдущему релизу без новых верхних action-кнопок и лишнего перехода загрузки карточки.

Проверки релиза:

- Обязателен полный Tier 4 updater gate с exact public `v2.0.10` на Windows VM через штатный `start_windows.bat` и UI updater.
- Physical Windows updater gate и проверка exact candidate выполнены до публикации; Telegram отправлен отдельным авторизованным шагом.
...
- В списке кавалеров появилась компактная алфавитная навигация внутри списка; повторные переходы, поиск и открытие карточек сохраняют рабочее состояние.
- Создание кавалера продолжается в его карточке: можно последовательно добавить фотографии и несколько наград, а завершение остаётся явным.
- В результатах поиска наград видна фотография кавалера; пустое состояние вкладки «Знаки» приведено к штатному тёмному стилю.
- В post-create карточке аккуратно выровнены ссылки и блок наград без изменения работы фотографий, медиа и наград.
- Сохранены ускоренная запись, непрерывные переходы без чёрных кадров, корректная сортировка и выбор созданного кавалера.

Проверки release candidate:

- Перед публикацией обязательны полный Tier 4 updater gate на Windows VM из exact public `v2.0.9` и ручной physical Windows Owner gate на том же candidate.
- Candidate package и manifest изолированы от production update channel; publication, public `latest.json` и Telegram заблокированы до Owner PASS.
...
- Ускорены создание и редактирование кавалеров с фотографиями без ослабления atomic media lifecycle.
- Добавлены немедленные состояния «Сохраняем…» и «Удаляем…» с защитой от повторной отправки.
- Устранены пустые и чёрные промежуточные кадры при сохранении, удалении и связанных переходах.
- После создания новый кавалер выбирается, прокручивается в область видимости и занимает правильную позицию активной сортировки.
- Сохранены выделение, прокрутка, фильтры и сортировка списка при обычной навигации.
- Ускорена готовность вкладки «Кавалеры» на больших базах: восстановление нулевой прокрутки больше не запускает лишний layout полного списка.

Проверки release candidate:

- Focused и full regression, package safety и exact updater gate выполняются в ALE-348 до публикации.
- Candidate проходит три независимых обновления с post-recovery public `v2.0.7` на Windows VM с постоянным mutable Sergey fixture.
- Публикация блокируется до ручного Owner gate на physical Windows.
