...
- После Owner feature PASS release stage не повторяет VM updater gate, full suite или product regression по умолчанию. После controlled merge и минимальной package/parity проверки обязательно подготовить изолированный Owner candidate channel и доказать на существующем physical `Public Current`, что штатная проверка обновлений видит exact candidate. До этого нельзя сообщать `READY FOR OWNER MANUAL UPDATE`. Production `latest.json` при этом не менять; подробности: `docs/OWNER_CANDIDATE_CHANNEL.md`.
