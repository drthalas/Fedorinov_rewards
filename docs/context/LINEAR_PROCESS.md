...
После Owner feature PASS release-candidate stage не повторяет Windows VM updater gate, full suite, headed product regression или старый branch preview по умолчанию. Он фиксирует accepted HEAD, выполняет controlled integration, следующую patch-версию и только минимальные version/manifest/SHA/package-safety/parity/static checks. Exact ZIP сразу размещается в изолированном Owner candidate channel, а постоянный physical `Public Current` через штатное `О программе → Проверить обновления` должен увидеть candidate без запуска update. Только после этой проверки разрешён статус `READY FOR OWNER MANUAL UPDATE`. Реальный pre-publication updater gate выполняет Owner из `Public Current`; VM допустима только при evidence-based mismatch или изменении updater/recovery/packaging/bootstrap/migration. Production channel остаётся на текущем public release. Каналы, deploy/restore и короткая publication stage описаны в `docs/OWNER_CANDIDATE_CHANNEL.md` и `docs/RELEASE_PROCESS.md`.
