...
Канонические discovery states, SYSTEM access watchdog, reboot/idle proof и
rollback определены только в `docs/testing/WINDOWS_PHYSICAL_GATE.md`. Этот
документ начинается после `SSH_READY` и не переопределяет connectivity/power
policy.
...
После idle проверять GUI только после отдельного connectivity preflight. Если
обычный SSH alias работает, но explicit preflight сообщает banner delay,
диагностировать OpenSSH pre-auth traffic и helper layer; не создавать новую
GUI-session и не выполнять reboot как способ скрыть transport failure.
