(function () {
  ...
  function showSavedMessage(form, blob, filename, mode) {
    if (mode === "fallback") {
      setMessage(
        form,
        "Файл скачан. Браузер не разрешает приложению автоматически открыть папку; откройте файл из загрузок браузера.",
        "success",
      );
    } else {
      setMessage(
        form,
        "Файл сохранён. Браузер не разрешает приложению автоматически открыть папку; откройте файл из выбранной папки или используйте ссылку “Открыть копию файла”.",
        "success",
      );
    }
